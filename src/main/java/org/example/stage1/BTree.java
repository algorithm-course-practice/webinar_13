 ...
 * @see <a href="https://github.com/phishman3579/java-algorithms-implementation/blob/master/src/com/jwetherell/algorithms/data_structures/BTree.java">Original Sources</a>
 * <br>
...
public class BTree<T extends Comparable<T>> {
    ...
    PrettyPrintTree<BTree.Node<T>> printer = new PrettyPrintTree<>(Node::getChildren, Node::toString);
    ...
    public java.util.Collection<T> inOrder() {
        List<T> res = new ArrayList<T>(size);
        inOrder(root, res);
        return res;
    }

    private void inOrder(Node<T> current, List<T> res) {
        if(current == null) return;
        int iChild = 0;
        int iKey = 0;
        while (iChild < current.childrenSize || iKey < current.keysSize){
            int currentChild = iChild++;
            int currentKey = iKey++;
            if(currentChild < current.childrenSize) {
                inOrder(current.getChild(currentChild), res);
            }
            if(currentKey < current.keysSize) {
                res.add(current.getKey(currentKey));
            }
        }
    }
